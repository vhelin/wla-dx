
case 0:
for ( ; x < OP_SIZE_MAX; inz++, x++) {
  if (g_opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
    output_assembled_opcode(g_opt_tmp, "d%d ", g_opt_tmp->hex);
    g_source_pointer = inz;
    return SUCCEEDED;
  }
  if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
    break;
 }
break;

case 1:
for ( ; x < OP_SIZE_MAX; inz++, x++) {
  if (g_opt_tmp->op[x] == 'x') {
    y = g_source_pointer;
    g_source_pointer = inz;
    z = input_number();
    inz = g_source_pointer;
    g_source_pointer = y;
    if (!(z == SUCCEEDED || z == INPUT_NUMBER_ADDRESS_LABEL || z == INPUT_NUMBER_STACK))
      return FAILED;
    if (z == SUCCEEDED && (d > 255 || d < -128))
      break;

    for (x++; x < OP_SIZE_MAX; inz++, x++) {
      if (g_opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
        if (z == SUCCEEDED)
          output_assembled_opcode(g_opt_tmp, "d%d d%d ", g_opt_tmp->hex, d);
        else if (z == INPUT_NUMBER_ADDRESS_LABEL)
          output_assembled_opcode(g_opt_tmp, "k%d d%d Q%s ", g_active_file_info_last->line_current, g_opt_tmp->hex, g_label);
        else {
          output_assembled_opcode(g_opt_tmp, "d%d c%d ", g_opt_tmp->hex, g_latest_stack);
        }

        g_source_pointer = inz;
        return SUCCEEDED;
      }
      if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
        break;
    }
  }
  if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
    break;
 }
break;

case 2:
for ( ; x < OP_SIZE_MAX; inz++, x++) {
  if (g_opt_tmp->op[x] == '?') {
    y = g_source_pointer;
    g_source_pointer = inz;
    z = input_number();
    inz = g_source_pointer;
    g_source_pointer = y;
    if (!(z == SUCCEEDED || z == INPUT_NUMBER_ADDRESS_LABEL || z == INPUT_NUMBER_STACK))
      return FAILED;
    if (z == SUCCEEDED && (d > 65535 || d < -32768)) {
      print_error("Out of 16-bit range.\n", ERROR_NUM);
      return FAILED;
    }

    for (x++; x < OP_SIZE_MAX; inz++, x++) {
      if (g_opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
        if (z == SUCCEEDED)
          output_assembled_opcode(g_opt_tmp, "d%d y%d ", g_opt_tmp->hex, d);
        else if (z == INPUT_NUMBER_ADDRESS_LABEL)
          output_assembled_opcode(g_opt_tmp, "k%d d%d r%s ", g_active_file_info_last->line_current, g_opt_tmp->hex, g_label);
        else
          output_assembled_opcode(g_opt_tmp, "d%d C%d ", g_opt_tmp->hex, g_latest_stack);

        g_source_pointer = inz;
        return SUCCEEDED;
      }
      if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
        break;
    }
  }
  if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
    break;
 }
break;

case 3:
for ( ; x < OP_SIZE_MAX; inz++, x++) {
  if (g_opt_tmp->op[x] == 'x') {
    y = g_source_pointer;
    g_source_pointer = inz;
    g_parse_floats = NO;
    z = input_number();
    g_parse_floats = YES;
    inz = g_source_pointer;
    g_source_pointer = y;
    if (!(z == SUCCEEDED || z == INPUT_NUMBER_ADDRESS_LABEL || z == INPUT_NUMBER_STACK))
      return FAILED;
    if (z == SUCCEEDED && (d > 255 || d < -128))
      break;

    e = d;
    v = z;
    h = g_latest_stack;
    if (z == INPUT_NUMBER_ADDRESS_LABEL)
      strcpy(labelx, g_label);

    for (x++; x < OP_SIZE_MAX; inz++, x++) {
      if (g_opt_tmp->op[x] == '~') {
        y = g_source_pointer;
        g_source_pointer = inz;
        z = input_number();
        inz = g_source_pointer;
        g_source_pointer = y;
        if (z != SUCCEEDED)
          break;
        if (d > 7 || d < 0)
          break;

        for (x++; x < OP_SIZE_MAX; inz++, x++) {
          if (g_opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
            output_assembled_opcode(g_opt_tmp, "d%d ", g_opt_tmp->hex | (d << 5));

            if (v == SUCCEEDED)
              output_assembled_opcode(g_opt_tmp, "d%d ", e);
            else if (v == INPUT_NUMBER_ADDRESS_LABEL)
              output_assembled_opcode(g_opt_tmp, "k%d Q%s ", g_active_file_info_last->line_current, labelx);
            else
              output_assembled_opcode(g_opt_tmp, "c%d ", h);

            g_source_pointer = inz;
            return SUCCEEDED;
          }
          if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
            break;
        }
      }
      if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
        break;
    }
  }
  if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
    break;
 }
break;

case 4:
for ( ; x < OP_SIZE_MAX; inz++, x++) {
  if (g_opt_tmp->op[x] == 'x') {
    y = g_source_pointer;
    g_source_pointer = inz;
    g_parse_floats = NO;
    z = input_number();
    g_parse_floats = YES;
    inz = g_source_pointer;
    g_source_pointer = y;
    if (!(z == SUCCEEDED || z == INPUT_NUMBER_ADDRESS_LABEL || z == INPUT_NUMBER_STACK))
      return FAILED;
    if (z == SUCCEEDED && (d > 255 || d < -128))
      break;

    e = d;
    v = z;
    h = g_latest_stack;
    if (z == INPUT_NUMBER_ADDRESS_LABEL)
      strcpy(labelx, g_label);

    for (x++; x < OP_SIZE_MAX; inz++, x++) {
      if (g_opt_tmp->op[x] == '~') {
        y = g_source_pointer;
        g_source_pointer = inz;
        z = input_number();
        inz = g_source_pointer;
        g_source_pointer = y;
        if (z != SUCCEEDED)
          break;
        if (d > 7 || d < 0)
          break;

        for (x++; x < OP_SIZE_MAX; inz++, x++) {
          if (g_opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
            output_assembled_opcode(g_opt_tmp, "d%d ", g_opt_tmp->hex | (((d << 1) + 1) << 4));

            if (v == SUCCEEDED)
              output_assembled_opcode(g_opt_tmp, "d%d ", e);
            else if (v == INPUT_NUMBER_ADDRESS_LABEL)
              output_assembled_opcode(g_opt_tmp, "k%d Q%s ", g_active_file_info_last->line_current, labelx);
            else
              output_assembled_opcode(g_opt_tmp, "c%d ", h);

            g_source_pointer = inz;
            return SUCCEEDED;
          }
          if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
            break;
        }
      }
      if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
        break;
    }
  }
  if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
    break;
 }
break;

case 5:
for ( ; x < OP_SIZE_MAX; inz++, x++) {
  if (g_opt_tmp->op[x] == '~') {
    y = g_source_pointer;
    g_source_pointer = inz;
    z = input_number();
    inz = g_source_pointer;
    g_source_pointer = y;
    if (z != SUCCEEDED)
      break;
    if (d > 0xF || d < 0)
      break;

    for (x++; x < OP_SIZE_MAX; inz++, x++) {
      if (g_opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
        output_assembled_opcode(g_opt_tmp, "d%d ", g_opt_tmp->hex | (d << 4));
        g_source_pointer = inz;
        return SUCCEEDED;
      }
      if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
        break;
    }
  }
  if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
    break;
 }
break;

case 0xA:
for ( ; x < OP_SIZE_MAX; inz++, x++) {
  if (g_opt_tmp->op[x] == 'x') {
    y = g_source_pointer;
    g_source_pointer = inz;
    z = input_number();
    inz = g_source_pointer;
    g_source_pointer = y;
    if (!(z == SUCCEEDED || z == INPUT_NUMBER_ADDRESS_LABEL || z == INPUT_NUMBER_STACK))
      return FAILED;
    if (z == SUCCEEDED && (d > 255 || d < -128))
      break;

    e = d;
    v = z;
    h = g_latest_stack;
    if (z == INPUT_NUMBER_ADDRESS_LABEL)
      strcpy(labelx, g_label);

    for (x++; x < OP_SIZE_MAX; inz++, x++) {
      if (g_opt_tmp->op[x] == 'x') {
        y = g_source_pointer;
        g_source_pointer = inz;
        z = input_number();
        inz = g_source_pointer;
        g_source_pointer = y;
        if (!(z == SUCCEEDED || z == INPUT_NUMBER_ADDRESS_LABEL || z == INPUT_NUMBER_STACK))
          return FAILED;
        if (z == SUCCEEDED && (d > 255 || d < -128))
          break;

        for (x++; x < OP_SIZE_MAX; inz++, x++) {
          if (g_opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
            if (v == SUCCEEDED)
              output_assembled_opcode(g_opt_tmp, "d%d d%d ", g_opt_tmp->hex, e);
            else if (v == INPUT_NUMBER_ADDRESS_LABEL)
              output_assembled_opcode(g_opt_tmp, "k%d d%d Q%s ", g_active_file_info_last->line_current, g_opt_tmp->hex, labelx);
            else
              output_assembled_opcode(g_opt_tmp, "d%d c%d ", g_opt_tmp->hex, h);

            if (z == SUCCEEDED)
              output_assembled_opcode(g_opt_tmp, "d%d ", d);
            else if (z == INPUT_NUMBER_ADDRESS_LABEL)
              output_assembled_opcode(g_opt_tmp, "k%d R%s ", g_active_file_info_last->line_current, g_label);
            else {
              output_assembled_opcode(g_opt_tmp, "c%d ", g_latest_stack);
              /* let's configure the stack so that all label references inside are relative */
              g_stacks_tmp->relative_references = 1;
            }

            g_source_pointer = inz;
            return SUCCEEDED;
          }
          if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
            break;
        }
      }
      if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
        break;
    }
  }
  if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
    break;
 }
break;

case 0xB:
for ( ; x < OP_SIZE_MAX; inz++, x++) {
  if (g_opt_tmp->op[x] == 'x') {
    y = g_source_pointer;
    g_source_pointer = inz;
    g_parse_floats = NO;
    z = input_number();
    g_parse_floats = YES;
    inz = g_source_pointer;
    g_source_pointer = y;
    if (!(z == SUCCEEDED || z == INPUT_NUMBER_ADDRESS_LABEL || z == INPUT_NUMBER_STACK))
      return FAILED;
    if (z == SUCCEEDED && (d > 255 || d < -128))
      break;

    e = d;
    v = z;
    h = g_latest_stack;
    if (z == INPUT_NUMBER_ADDRESS_LABEL)
      strcpy(labelx, g_label);

    for (x++; x < OP_SIZE_MAX; inz++, x++) {
      if (g_opt_tmp->op[x] == 'x') {
        y = g_source_pointer;
        g_source_pointer = inz;
        z = input_number();
        inz = g_source_pointer;
        g_source_pointer = y;
        if (!(z == SUCCEEDED || z == INPUT_NUMBER_ADDRESS_LABEL || z == INPUT_NUMBER_STACK))
          return FAILED;
        if (z == SUCCEEDED && (d > 255 || d < -128))
          break;

        for (x++; x < OP_SIZE_MAX; inz++, x++) {
          if (g_opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
            if (z == SUCCEEDED)
              output_assembled_opcode(g_opt_tmp, "d%d d%d ", g_opt_tmp->hex, d);
            else if (z == INPUT_NUMBER_ADDRESS_LABEL)
              output_assembled_opcode(g_opt_tmp, "k%d d%d Q%s ", g_active_file_info_last->line_current, g_opt_tmp->hex, g_label);
            else
              output_assembled_opcode(g_opt_tmp, "d%d c%d ", g_opt_tmp->hex, g_latest_stack);

            if (v == SUCCEEDED)
              output_assembled_opcode(g_opt_tmp, "d%d ", e);
            else if (v == INPUT_NUMBER_ADDRESS_LABEL)
              output_assembled_opcode(g_opt_tmp, "k%d Q%s ", g_active_file_info_last->line_current, labelx);
            else
              output_assembled_opcode(g_opt_tmp, "c%d ", h);

            g_source_pointer = inz;
            return SUCCEEDED;
          }
          if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
            break;
        }
      }
      if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
        break;
    }
  }
  if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
    break;
 }
break;

case 0xC:
for ( ; x < OP_SIZE_MAX; inz++, x++) {
  if (g_opt_tmp->op[x] == 'x') {
    y = g_source_pointer;
    g_source_pointer = inz;
    g_parse_floats = NO;
    z = input_number();
    g_parse_floats = YES;
    inz = g_source_pointer;
    g_source_pointer = y;
    if (!(z == SUCCEEDED || z == INPUT_NUMBER_ADDRESS_LABEL || z == INPUT_NUMBER_STACK))
      return FAILED;
    if (z == SUCCEEDED && (d > 255 || d < -128))
      break;

    e = d;
    v = z;
    h = g_latest_stack;
    if (z == INPUT_NUMBER_ADDRESS_LABEL)
      strcpy(labelx, g_label);

    for (x++; x < OP_SIZE_MAX; inz++, x++) {
      if (g_opt_tmp->op[x] == '~') {
        y = g_source_pointer;
        g_source_pointer = inz;
        z = input_number();
        inz = g_source_pointer;
        g_source_pointer = y;
        if (z != SUCCEEDED)
          break;
        if (d > 7 || d < 0)
          break;
        g = d;

        for (x++; x < OP_SIZE_MAX; inz++, x++) {
          if (g_opt_tmp->op[x] == 'x') {
            y = g_source_pointer;
            g_source_pointer = inz;
            z = input_number();
            inz = g_source_pointer;
            g_source_pointer = y;
            if (!(z == SUCCEEDED || z == INPUT_NUMBER_ADDRESS_LABEL || z == INPUT_NUMBER_STACK))
              return FAILED;
            if (z == SUCCEEDED && (d > 255 || d < -128))
              break;

            for (x++; x < OP_SIZE_MAX; inz++, x++) {
              if (g_opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
                output_assembled_opcode(g_opt_tmp, "d%d ", g_opt_tmp->hex | (g << 5));

                if (v == SUCCEEDED)
                  output_assembled_opcode(g_opt_tmp, "d%d ", e);
                else if (v == INPUT_NUMBER_ADDRESS_LABEL)
                  output_assembled_opcode(g_opt_tmp, "k%d Q%s ", g_active_file_info_last->line_current, labelx);
                else
                  output_assembled_opcode(g_opt_tmp, "c%d ", h);

                if (z == SUCCEEDED)
                  output_assembled_opcode(g_opt_tmp, "d%d ", d);
                else if (z == INPUT_NUMBER_ADDRESS_LABEL)
                  output_assembled_opcode(g_opt_tmp, "k%d R%s ", g_active_file_info_last->line_current, g_label);
                else {
                  output_assembled_opcode(g_opt_tmp, "c%d ", g_latest_stack);
                  /* let's configure the stack so that all label references inside are relative */
                  g_stacks_tmp->relative_references = 1;
                }

                g_source_pointer = inz;
                return SUCCEEDED;
              }
              if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
                break;
            }
          }
          if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
            break;
        }
      }
      if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
        break;
    }
  }
  if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
    break;
 }
break;

case 0xD:
for ( ; x < OP_SIZE_MAX; inz++, x++) {
  if (g_opt_tmp->op[x] == 'x') {
    y = g_source_pointer;
    g_source_pointer = inz;
    g_parse_floats = NO;
    z = input_number();
    g_parse_floats = YES;
    inz = g_source_pointer;
    g_source_pointer = y;
    if (!(z == SUCCEEDED || z == INPUT_NUMBER_ADDRESS_LABEL || z == INPUT_NUMBER_STACK))
      return FAILED;
    if (z == SUCCEEDED && (d > 255 || d < -128))
      break;

    e = d;
    v = z;
    h = g_latest_stack;
    if (z == INPUT_NUMBER_ADDRESS_LABEL)
      strcpy(labelx, g_label);

    for (x++; x < OP_SIZE_MAX; inz++, x++) {
      if (g_opt_tmp->op[x] == '~') {
        y = g_source_pointer;
        g_source_pointer = inz;
        z = input_number();
        inz = g_source_pointer;
        g_source_pointer = y;
        if (z != SUCCEEDED)
          break;
        if (d > 7 || d < 0)
          break;
        g = d;

        for (x++; x < OP_SIZE_MAX; inz++, x++) {
          if (g_opt_tmp->op[x] == 'x') {
            y = g_source_pointer;
            g_source_pointer = inz;
            z = input_number();
            inz = g_source_pointer;
            g_source_pointer = y;
            if (!(z == SUCCEEDED || z == INPUT_NUMBER_ADDRESS_LABEL || z == INPUT_NUMBER_STACK))
              return FAILED;
            if (z == SUCCEEDED && (d > 255 || d < -128))
              break;

            for (x++; x < OP_SIZE_MAX; inz++, x++) {
              if (g_opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
                output_assembled_opcode(g_opt_tmp, "d%d ", g_opt_tmp->hex | (((g << 1) + 1) << 4));

                if (v == SUCCEEDED)
                  output_assembled_opcode(g_opt_tmp, "d%d ", e);
                else if (v == INPUT_NUMBER_ADDRESS_LABEL)
                  output_assembled_opcode(g_opt_tmp, "k%d Q%s ", g_active_file_info_last->line_current, labelx);
                else
                  output_assembled_opcode(g_opt_tmp, "c%d ", h);

                if (z == SUCCEEDED)
                  output_assembled_opcode(g_opt_tmp, "d%d ", d);
                else if (z == INPUT_NUMBER_ADDRESS_LABEL)
                  output_assembled_opcode(g_opt_tmp, "k%d R%s ", g_active_file_info_last->line_current, g_label);
                else {
                  output_assembled_opcode(g_opt_tmp, "c%d ", g_latest_stack);
                  /* let's configure the stack so that all label references inside are relative */
                  g_stacks_tmp->relative_references = 1;
                }

                g_source_pointer = inz;
                return SUCCEEDED;
              }
              if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
                break;
            }
          }
          if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
            break;
        }
      }
      if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
        break;
    }
  }
  if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
    break;
 }
break;

case 0xE:
for ( ; x < OP_SIZE_MAX; inz++, x++) {
  if (g_opt_tmp->op[x] == 'x') {
    y = g_source_pointer;
    g_source_pointer = inz;
    z = input_number();
    inz = g_source_pointer;
    g_source_pointer = y;
    if (!(z == SUCCEEDED || z == INPUT_NUMBER_ADDRESS_LABEL || z == INPUT_NUMBER_STACK))
      return FAILED;
    if (z == SUCCEEDED && (d > 255 || d < -128))
      break;

    for (x++; x < OP_SIZE_MAX; inz++, x++) {
      if (g_opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
        if (z == SUCCEEDED)
          output_assembled_opcode(g_opt_tmp, "d%d d%d ", g_opt_tmp->hex, d);
        else if (z == INPUT_NUMBER_ADDRESS_LABEL)
          output_assembled_opcode(g_opt_tmp, "k%d d%d R%s ", g_active_file_info_last->line_current, g_opt_tmp->hex, g_label);
        else {
          output_assembled_opcode(g_opt_tmp, "d%d c%d ", g_opt_tmp->hex, g_latest_stack);
          g_stacks_tmp->relative_references = 1;
        }

        g_source_pointer = inz;
        return SUCCEEDED;
      }
      if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
        break;
    }
  }
  if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
    break;
 }
break;

case 0xF:
for ( ; x < OP_SIZE_MAX; inz++, x++) {
  if (g_opt_tmp->op[x] == '?') {
    y = g_source_pointer;
    g_source_pointer = inz;
    g_parse_floats = NO;
    z = input_number();
    g_parse_floats = YES;
    inz = g_source_pointer;
    g_source_pointer = y;
    if (!(z == SUCCEEDED || z == INPUT_NUMBER_ADDRESS_LABEL || z == INPUT_NUMBER_STACK))
      return FAILED;
    if (z == SUCCEEDED && (d > 8191 || d < 0))
      break;

    e = d;
    v = z;
    h = g_latest_stack;
    if (z == INPUT_NUMBER_ADDRESS_LABEL)
      strcpy(labelx, g_label);

    for (x++; x < OP_SIZE_MAX; inz++, x++) {
      if (g_opt_tmp->op[x] == '~') {
        y = g_source_pointer;
        g_source_pointer = inz;
        z = input_number();
        inz = g_source_pointer;
        g_source_pointer = y;
        if (z != SUCCEEDED)
          break;
        if (d > 7 || d < 0)
          break;

        for (x++; x < OP_SIZE_MAX; inz++, x++) {
          if (g_opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
            if (v == SUCCEEDED)
              output_assembled_opcode(g_opt_tmp, "d%d y%d ", g_opt_tmp->hex, e | d << 13);
            else if (v == INPUT_NUMBER_ADDRESS_LABEL)
              output_assembled_opcode(g_opt_tmp, "k%d d%d n%d %s ", g_active_file_info_last->line_current, g_opt_tmp->hex, d, labelx);
            else
              output_assembled_opcode(g_opt_tmp, "d%d N%d %d ", g_opt_tmp->hex, d, h);

            g_source_pointer = inz;
            return SUCCEEDED;
          }
          if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
            break;
        }
      }
      if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
        break;
    }
  }
  if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
    break;
 }
break;

