...
if ! [ -x "$(command -v valgrind)" ]; then
  ...
  export WLAVALGRIND='valgrind --error-exitcode=1 --tool=memcheck --leak-check=full --errors-for-leak-kinds=all --track-origins=yes --read-var-info=yes --show-error-list=yes --show-leak-kinds=all --expensive-definedness-checks=yes --keep-debuginfo=yes -s'
fi
