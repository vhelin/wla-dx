  ...
  if (g_opt_tmp->op[x] == 's') {
    ...
    for (x++ ; x < OP_SIZE_MAX; inz++, x++) {
      if (g_opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
        ...
        output_assembled_opcode(g_opt_tmp, "d%d ", g_opt_tmp->addressing_mode_bits | (d & 0x1F));
        g_source_pointer = inz;
        return SUCCEEDED;
      }
      if (g_opt_tmp->op[x] != toupper((int)g_buffer[inz]))
  ...
  if (g_opt_tmp->op[x] == 'x') {
    ...
    for (x++ ; x < OP_SIZE_MAX; inz++, x++) {
      if (g_opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
        ...
        if (z == SUCCEEDED)
          output_assembled_opcode(g_opt_tmp, "d%d d%d ", g_opt_tmp->addressing_mode_bits, d);
        else if (z == INPUT_NUMBER_ADDRESS_LABEL)
  ...
  if (g_opt_tmp->op[x] == '?') {
    ...
    for (x++ ; x < OP_SIZE_MAX; inz++, x++) {
      if (g_opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
        ...
        if (z == SUCCEEDED)
          output_assembled_opcode(g_opt_tmp, "d%d y%d ", g_opt_tmp->addressing_mode_bits, d);
        else if (z == INPUT_NUMBER_ADDRESS_LABEL)
          output_assembled_opcode(g_opt_tmp, "d%d r%s ", g_opt_tmp->addressing_mode_bits, g_label);
        else
          output_assembled_opcode(g_opt_tmp, "d%d C%d ", g_opt_tmp->addressing_mode_bits, g_latest_stack);
...
for ( ; x < OP_SIZE_MAX; inz++, x++) {
  if (g_opt_tmp->op[x] == 'r') {
    d = parse_push_pull_registers(g_opt_tmp->addressing_mode_bits);
    if (d < 0)
      return FAILED;
