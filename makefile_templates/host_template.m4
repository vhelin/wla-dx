...
define(`SHEBANG', `dnl')dnl A script prologue/header. No arguments required. 
dnl Safe to leave as "dnl" on non-Unix systems, but "@echo off" is recommended
dnl for DOS/Windows.
define(`CP', )dnl Copy command. Expects two arguments: source ($1), dest ($2).
define(`CHDIR', )dnl Chdir command.
define(`RM', )dnl Remove file command.
define(`RMFLAGS', `dnl')dnl Reserved. Safe to leave as "dnl".
define(`EXEC', )dnl How do you execute EXEs- by calling the exe name directly 
dnl or prepending the current directory (./), or some other means?
define(`SHEXEC', )dnl How do you execute one shell script from another- by 
dnl naming the script directly, prepending the current directory (./), or some 
dnl other means?
define(`ECHO', )dnl Print to screen command.
define(`CHMOD', `dnl')dnl It is safe to leave as "dnl" on non-Unix systems.
define(`MAKE', )dnl Make command. If your environment supports parallel make, 
...
define(`EXPORT', )dnl Set environment variables command. Expects
...
define(`EXIT', )dnl Exit command. Input argument optional?
dnl
dnl OS-specific syntax
define(`COMMENT', )dnl What is the line comment character for scripts?
define(`SH_EXTENSION', )dnl What extension do shell scripts/command 
dnl files have?
define(`OBJ_EXTENSION', )dnl Object file exten
define(`EXE_EXTENSION', )dnl Dot required here, since Unix doesn't use 
dnl extensions for exes...
define(`DEFAULT_DEST', )dnl Not currently used, but where is the final 
dnl destination of binaries?
define(`PATH_SEPARATOR', )dnl Redundant?
dnl
dnl Describe your C compiler
define(`CC', )dnl C Compiler name
define(`CFLAGS_DEFINE', )dnl Switch to define preprocessor define. Takes one 
dnl argument.
define(`CFLAGS_HOST', )dnl Use CFLAGS_DEFINE to add 
...
define(`CLAGS_COMPILE', )dnl Switch to compile only.
define(`CFLAGS_OPT', )dnl Switch to optimize. 
define(`CFLAGS_ANSI', )dnl Warnings and ANSI compliance switches.
...
define(`LD', )dnl Linker name.
define(`LDFLAGS_OTHER', )dnl Miscellaneous switches.
