          ...
          for (x++; x < INSTRUCTION_STRING_LENGTH_MAX; g_inz++, x++) {
            if (g_instruction_tmp->string[x] == 0 && g_buffer[g_inz] == 0x0A) {
              if (z == SUCCEEDED)
                ...
                output_assembled_instruction(g_instruction_tmp, "d%d T%d ", g_instruction_tmp->hex, g_latest_stack);
              g_source_pointer = g_inz;
              return SUCCEEDED;
            }
            if (g_instruction_tmp->string[x] != toupper((int)g_buffer[g_inz]))
    ...
    case 100:
      ...
      for ( ; x < INSTRUCTION_STRING_LENGTH_MAX; g_inz++, x++) {
        if (g_instruction_tmp->string[x] == '*') {
          ...
          for (x++ ; x < INSTRUCTION_STRING_LENGTH_MAX; g_inz++, x++) {
            if (g_instruction_tmp->string[x] == 0 && g_buffer[g_inz] == 0x0A) {
              output_assembled_instruction(g_instruction_tmp, "k%d v2 ", g_active_file_info_last->line_current);
              if (z == INPUT_NUMBER_ADDRESS_LABEL)
                output_assembled_instruction(g_instruction_tmp, "Q%s ", g_label);
              else
                output_assembled_instruction(g_instruction_tmp, "c%d ", g_latest_stack);       
      ...
      for ( ; x < INSTRUCTION_STRING_LENGTH_MAX; g_inz++, x++) {
        if (g_instruction_tmp->string[x] == 0 && g_buffer[g_inz] == 0x0A) {
          ...
          if (g_instruction_tmp->prefix != 0)
            output_assembled_instruction(g_instruction_tmp, "d%d ", g_instruction_tmp->prefix);
        ...
        if (g_instruction_tmp->string[x] == '*') {
          ...
          for (x++; x < INSTRUCTION_STRING_LENGTH_MAX; g_inz++, x++) {
            if (g_instruction_tmp->string[x] == 0 && g_buffer[g_inz] == 0x0A) {
              ...
              output_assembled_instruction(g_instruction_tmp, "d%d ", g_instruction_tmp->hex | tiny);
            ...
            if (g_instruction_tmp->string[x] == 'x') {
              ...
              for (x++ ; x < INSTRUCTION_STRING_LENGTH_MAX; g_inz++, x++) {
                if (g_instruction_tmp->string[x] == 0 && g_buffer[g_inz] == 0x0A) {
                  ...
                  if (z == SUCCEEDED)
                    output_assembled_instruction(g_instruction_tmp, "d%d d%d ", g_instruction_tmp->hex | tiny, g_parsed_int);
                  else if (z == INPUT_NUMBER_ADDRESS_LABEL)
            ...
            if (g_instruction_tmp->string[x] == '?') {
              ...
              for (x++ ; x < INSTRUCTION_STRING_LENGTH_MAX; g_inz++, x++) {
                if (g_instruction_tmp->string[x] == 0 && g_buffer[g_inz] == 0x0A) {
                  ...
                  if (z == SUCCEEDED)
                    output_assembled_instruction(g_instruction_tmp, "d%d y%d ", g_instruction_tmp->hex | tiny, g_parsed_int);
                  else if (z == INPUT_NUMBER_ADDRESS_LABEL)
                    output_assembled_instruction(g_instruction_tmp, "d%d r%s ", g_instruction_tmp->hex | tiny, g_label);
                  else
                    output_assembled_instruction(g_instruction_tmp, "d%d C%d ", g_instruction_tmp->hex | tiny, g_latest_stack);
            ...
            if (g_instruction_tmp->string[x] == 'y') {
              ...
              for (x++ ; x < INSTRUCTION_STRING_LENGTH_MAX; g_inz++, x++) {
                if (g_instruction_tmp->string[x] == 0 && g_buffer[g_inz] == 0x0A) {
                  ...
                  if (z == SUCCEEDED)
                    output_assembled_instruction(g_instruction_tmp, "d%d d%d ", g_instruction_tmp->hex | tiny, g_parsed_int >> 1);
                  else if (z == INPUT_NUMBER_ADDRESS_LABEL)
                    output_assembled_instruction(g_instruction_tmp, "d%d *%s ", g_instruction_tmp->hex | tiny, g_label);
                  else
                    output_assembled_instruction(g_instruction_tmp, "d%d -%d ", g_instruction_tmp->hex | tiny, g_latest_stack);
              ...
              for (x++ ; x < INSTRUCTION_STRING_LENGTH_MAX; g_inz++, x++) {
                if (g_instruction_tmp->string[x] == 0 && g_buffer[g_inz] == 0x0A) {
                  ...
                  if (g_instruction_tmp->hex == 0xB0) {
                    /* MOVES */
                    output_assembled_instruction(g_instruction_tmp, "d%d ", g_instruction_tmp->prefix | tiny);
                    output_assembled_instruction(g_instruction_tmp, "d%d ", g_instruction_tmp->hex | z);
                  }
                  else {
                    /* OTHERWISE */
                    output_assembled_instruction(g_instruction_tmp, "d%d ", g_instruction_tmp->prefix | z);
                    output_assembled_instruction(g_instruction_tmp, "d%d ", g_instruction_tmp->hex | tiny);
                  }
                  g_source_pointer = g_inz;
                  return SUCCEEDED;
                ...
                if (g_instruction_tmp->string[x] == 0 && g_buffer[g_inz] == 0x0A) {
                  ...
                  if (z == SUCCEEDED) {
                    if (g_parsed_int >= -128 && g_parsed_int <= 127) {
                      /* use IBT */
                      output_assembled_instruction(g_instruction_tmp, "d%d d%d ", 0xA0 | tiny, g_parsed_int);
                    }
                    else {
                      /* use IWT */
                      output_assembled_instruction(g_instruction_tmp, "d%d y%d ", 0xF0 | tiny, g_parsed_int);
                    }
                  }
                  else if (z == INPUT_NUMBER_ADDRESS_LABEL)
                    output_assembled_instruction(g_instruction_tmp, "d%d r%s ", 0xF0 | tiny, g_label);
                  else
                    output_assembled_instruction(g_instruction_tmp, "d%d C%d ", 0xF0 | tiny, g_latest_stack);
                ...
                if (g_instruction_tmp->string[x] == 0 && g_buffer[g_inz] == 0x0A) {
                  ...
                  if (z == SUCCEEDED) {
                    if ((g_parsed_int >= 0 && g_parsed_int <= 510) && ((g_parsed_int & 1) == 0)) {
                      /* use LMS */
                      output_assembled_instruction(g_instruction_tmp, "d%d d%d d%d ", 0x3D, 0xA0 | tiny, g_parsed_int >> 1);
                    }
                    else {
                      /* use LM */
                      output_assembled_instruction(g_instruction_tmp, "d%d d%d y%d ", 0x3D, 0xF0 | tiny, g_parsed_int);
                    }
                  }
                  else if (z == INPUT_NUMBER_ADDRESS_LABEL)
                    output_assembled_instruction(g_instruction_tmp, "d%d d%d r%s ", 0x3D, 0xF0 | tiny, g_label);
                  else
                    output_assembled_instruction(g_instruction_tmp, "d%d d%d C%d ", 0x3D, 0xF0 | tiny, g_latest_stack);
              ...
              for (x++ ; x < INSTRUCTION_STRING_LENGTH_MAX; g_inz++, x++) {
                if (g_instruction_tmp->string[x] == 0 && g_buffer[g_inz] == 0x0A) {
                  ...
                  if (z == SUCCEEDED) {
                    if ((y >= 0 && y <= 510) && ((y & 1) == 0)) {
                      /* use SMS */
                      output_assembled_instruction(g_instruction_tmp, "d%d d%d d%d ", 0x3E, 0xA0 | tiny, y >> 1);
                    }
                    else {
                      /* use SM */
                      output_assembled_instruction(g_instruction_tmp, "d%d d%d y%d ", 0x3E, 0xF0 | tiny, y);
                    }
                  }
                  else if (z == INPUT_NUMBER_ADDRESS_LABEL)
                    output_assembled_instruction(g_instruction_tmp, "d%d d%d r%s ", 0x3E, 0xF0 | tiny, g_label);
                  else
                    output_assembled_instruction(g_instruction_tmp, "d%d d%d C%d ", 0x3E, 0xF0 | tiny, g_latest_stack);
            ...
            if (g_instruction_tmp->string[x] == '*') {
              ...
              for (x++ ; x < INSTRUCTION_STRING_LENGTH_MAX; g_inz++, x++) {
                if (g_instruction_tmp->string[x] == 0 && g_buffer[g_inz] == 0x0A) {
                  ...
                  if (tiny == 0) /* LDB (R*) */
                    output_assembled_instruction(g_instruction_tmp, "d%d d%d ", 0x3D, 0x40 | z);
                  else /* TO R* + LDB (R*) */
                    output_assembled_instruction(g_instruction_tmp, "d%d d%d d%d ", 0x10 | tiny, 0x3D, 0x40 | z);
            ...
            if (g_instruction_tmp->string[x] == '*') {
              ...
              for (x++ ; x < INSTRUCTION_STRING_LENGTH_MAX; g_inz++, x++) {
                if (g_instruction_tmp->string[x] == 0 && g_buffer[g_inz] == 0x0A) {
                  ...
                  if (z == 0) /* STB (R*) */
                    output_assembled_instruction(g_instruction_tmp, "d%d d%d ", 0x3D, 0x30 | tiny);
                  else /* FROM R* + STB (R*) */
                    output_assembled_instruction(g_instruction_tmp, "d%d d%d d%d ", 0xB0 | z, 0x3D, 0x30 | tiny);
            ...
            if (g_instruction_tmp->string[x] == '*') {
              ...
              for (x++ ; x < INSTRUCTION_STRING_LENGTH_MAX; g_inz++, x++) {
                if (g_instruction_tmp->string[x] == 0 && g_buffer[g_inz] == 0x0A) {
                  ...
                  if (tiny == 0) /* LDW (R*) */
                    output_assembled_instruction(g_instruction_tmp, "d%d ", 0x40 | z);
                  else /* TO R* + LDW (R*) */
                    output_assembled_instruction(g_instruction_tmp, "d%d d%d ", 0x10 | tiny, 0x40 | z);
            ...
            if (g_instruction_tmp->string[x] == '*') {
              ...
              for (x++ ; x < INSTRUCTION_STRING_LENGTH_MAX; g_inz++, x++) {
                if (g_instruction_tmp->string[x] == 0 && g_buffer[g_inz] == 0x0A) {
                  ...
                  if (z == 0) /* STW (R*) */
                    output_assembled_instruction(g_instruction_tmp, "d%d ", 0x30 | tiny);
                  else /* FROM R* + STW (R*) */
                    output_assembled_instruction(g_instruction_tmp, "d%d d%d ", 0xB0 | z, 0x30 | tiny);
              ...
              if (g_instruction_tmp->string[x] == '*') {
                ...
                for (x++ ; x < INSTRUCTION_STRING_LENGTH_MAX; g_inz++, x++) {
                  if (g_instruction_tmp->string[x] == 0 && g_buffer[g_inz] == 0x0A) {
                    ...
                    if (z == SUCCEEDED)
                      output_assembled_instruction(g_instruction_tmp, "d%d y%d ", g_instruction_tmp->hex | tiny, v);
                    else if (z == INPUT_NUMBER_ADDRESS_LABEL)
                      output_assembled_instruction(g_instruction_tmp, "d%d r%s ", g_instruction_tmp->hex | tiny, labelx);
                    else
                      output_assembled_instruction(g_instruction_tmp, "d%d C%d ", g_instruction_tmp->hex | tiny, s);
              ...
              if (g_instruction_tmp->string[x] == '*') {
                ...
                for (x++ ; x < INSTRUCTION_STRING_LENGTH_MAX; g_inz++, x++) {
                  if (g_instruction_tmp->string[x] == 0 && g_buffer[g_inz] == 0x0A) {
                    ...
                    if (z == SUCCEEDED)
                      output_assembled_instruction(g_instruction_tmp, "d%d d%d ", g_instruction_tmp->hex | tiny, v >> 1);
                    else if (z == INPUT_NUMBER_ADDRESS_LABEL)
                      output_assembled_instruction(g_instruction_tmp, "d%d *%s ", g_instruction_tmp->hex | tiny, labelx);
                    else
                      output_assembled_instruction(g_instruction_tmp, "d%d -%d ", g_instruction_tmp->hex | tiny, s);
