    ...
    for (x++; x < OP_SIZE_MAX; inz++, x++) {
      if (opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
        if (z == SUCCEEDED)
          ...
          output_assembled_opcode(opt_tmp, "k%d y%d R%s ", g_active_file_info_last->line_current, opt_tmp->hex, g_label);
        else
    ...
    for (x++ ; x < OP_SIZE_MAX; inz++, x++) {
      if (opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
        if (z == SUCCEEDED)
          ...
          output_assembled_opcode(opt_tmp, "k%d y%d R%s d%d ", g_active_file_info_last->line_current, opt_tmp->hex, g_label, opt_tmp->hex_x);
        else
        ...
        for (x++ ; x < OP_SIZE_MAX; inz++, x++) {
          if (opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
            if (v == SUCCEEDED)
              ...
              output_assembled_opcode(opt_tmp, "k%d y%d R%s ", g_active_file_info_last->line_current, opt_tmp->hex, labelx);
            else
...
for ( ; x < OP_SIZE_MAX; inz++, x++) {
  if (opt_tmp->op[x] == '*') {
    ...
    for (x++ ; x < OP_SIZE_MAX; inz++, x++) {
      if (opt_tmp->op[x] == 0 && g_buffer[inz] == 0x0A) {
        output_assembled_opcode(opt_tmp, "k%d v1 ", g_active_file_info_last->line_current);
        if (z == INPUT_NUMBER_ADDRESS_LABEL)
